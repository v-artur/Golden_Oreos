...
- The <b>Modeling1.ipynb</b> notebook contains the modeling for the one speaker model with 3 neural network structures (more detail below). The notebook can be run after extracting the data, all the important steps of the training and the testing/evaluating will be executed automatically.
- The <b>"speaker_indep_dim_reduction.ipynb"</b> notebook was created for transforming the higher dimensional data for the speaker-independent model into lower dimensional data. Because of the dimensionality and the amount of the original data, if you are planning to run it, <b>you need to use Colab Pro with additional RAM and GPU</b> otherwise the notebook will run out of memory. After obtaining and extracting the data, you can run the rest of notebook, the imprortant steps will be executed automatically.
